...
@interface THDatePickerViewController () <UITableViewDelegate> {
...
@property (nonatomic, strong) ACYearDataSource* yearDataSource;
...
- (void)configureButtonAppearances {
  ...
  self.okButtonHidden = self.okButtonHidden;
}
...
-(void) setOkButtonHidden:(BOOL)okButtonHidden {
  _okButtonHidden = okButtonHidden;
  self.okBtn.hidden = okButtonHidden;
}
...
- (IBAction)monthButtonPressed:(id)sender {
  self.yearTableView = [[UITableView alloc] initWithFrame:CGRectZero style:UITableViewStylePlain];
  [self.controlView addSubview:self.yearTableView];
  self.yearDataSource = [ACYearDataSource new];
  self.yearDataSource.startYear = 1900;
  self.yearDataSource.endYear = 2200;
  self.yearTableView.dataSource = self.yearDataSource;
  ...
  [self.yearTableView autoPinEdgesToSuperviewMarginsExcludingEdge:ALEdgeTop];
  [self.yearTableView autoConstrainAttribute:ALAttributeTop toAttribute:ALAttributeTop ofView:self.monthButton withMultiplier:1];
  
  NSDateComponents* components = [_calendar components:NSCalendarUnitYear fromDate:self.firstOfCurrentMonth];
  [self.yearTableView reloadData];
  NSUInteger row = components.year - self.yearDataSource.startYear;
  [self.yearTableView selectRowAtIndexPath:[NSIndexPath indexPathForRow:row inSection:0] animated:YES scrollPosition:UITableViewScrollPositionMiddle];
}

-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
  [self.yearTableView removeFromSuperview];
  NSUInteger selectedYear = self.yearDataSource.startYear + indexPath.row;
  
  NSDateComponents* components = [_calendar components:NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay fromDate:self.firstOfCurrentMonth];
